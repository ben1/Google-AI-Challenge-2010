...
class Precalc
{
    public class PlanetData
    {
        ...
        public System.Collections.Generic.List<double> m_invDistancesD;
    ...
    public void Init(PlanetWars pw)
    {
        ...
        foreach (Planet p in pw.m_planets)
        {
            ...
            pd.m_invDistancesD = new List<double>(numPlanets);
            ...
            for (int i = 0; i < p.m_planetID; ++i)
            {
                ...
                pd.m_invDistancesD.Add(m_planetData[i].m_invDistancesD[p.m_planetID]);
                pd.m_distances.Add(m_planetData[i].m_distances[p.m_planetID]);
            }

            pd.m_distancesD.Add(1.0);
            pd.m_invDistancesD.Add(1.0);
            pd.m_distances.Add((int)1);
            ...
            for (int i = p.m_planetID + 1; i < numPlanets; ++i)
            {
                ...
                double dist = Math.Max(1.0, Math.Ceiling(Math.Sqrt(dx * dx + dy * dy)));
                pd.m_distancesD.Add(dist);
                pd.m_invDistancesD.Add(1.0 / dist);
