...
public class MyBot 
{
  ...
  public MyBot()
  {
    ...
    m_growthMul = 100.0;
    m_myPlanetsDistMul = 300.0;
    m_enemyPlanetsDistMul = 100.0;
    m_costMul = 12.0f;
    ...
    for (int targetPlanetIteration = 1; targetPlanetIteration <= sortedScores.Count && m_state.m_myPlanetsWithShips.Count > 0; ++targetPlanetIteration)
    {
      ...
      while (shipsToSend > 0 && m_state.m_myPlanetsWithShips.Count > 0)
      {
        ...
        for (int i = 0; i < m_state.m_myPlanetsWithShips.Count; ++i)
        {
          ...
          if(targetPlanet.m_eventualOwner == 2 && (targetPlanet.m_score - (m_enemyPlanetMul - 1.0) * (targetPlanet.GrowthRate() * m_growthMul)) > 1.3 * p.m_score) // remove the enemyPlanetMul component of the score
          {
