...
//    m_growthMul = double.Parse(a_args[0]);
  ...
  public void DoTurn(PlanetWars a_pw) 
  {
    ...
    for (int targetPlanetIteration = 1; targetPlanetIteration <= sortedScores.Count && m_state.m_myPlanetsWithShips.Count > 0; ++targetPlanetIteration)
    {
      ...
      if (targetPlanet.m_owner == 0 && (int)((double)targetPlanetIteration * 1.9) > sortedScores.Count)
      {
        break;
      }
      if (targetPlanet.m_owner == 0 && targetPlanet.m_score < targetPlanet.m_growthRate * -10)
      {
        break;
      }
      ...
      {
        int cepid = m_state.GetClosestEnemyPlanet(targetPlanet.m_planetID);
        if (cepid >= 0)
        {
          Planet e = a_pw.GetPlanet(cepid);
          if (e.m_score < targetPlanet.m_score)
          {
      ...
      while (m_state.m_myPlanetsWithShips.Count > 0)
      {
        ...
        for (int i = 0; i < m_state.m_myPlanetsWithShips.Count; ++i)
        {
          ...
          if (targetPlanet.m_owner == 0)
          {
            ...
            //bool stop = false;
            //foreach (Fleet f in targetPlanet.m_actualIncomingFleets)
            //{
            //  if (f.m_owner == 2 && f.m_turnsRemaining <= distances[pid])
            //  {
            //    stop = true;
            //    break;
            //  }
