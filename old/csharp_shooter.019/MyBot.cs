...
public class MyBot 
{
  ...
  public MyBot(string a_opts)
  {
    ...
    m_growthMul = m_opts[0];
    m_myPlanetsDistMul = m_opts[1];
    m_enemyPlanetMul = m_opts[2];
  }
#endif
...
//      if ((double)targetPlanetIteration * (m_opts[0] - ((double)m_state.m_current.m_turn / m_opts[1])) > (double)sortedScores.Count)
      if ((double)targetPlanetIteration * (2.3 - ((double)m_state.m_current.m_turn / 190)) > (double)sortedScores.Count)
      {
        break;
      }
...
//      if (targetPlanet.m_score < targetPlanet.m_growthRate * -m_opts[0] + m_opts[1])
      ...
      while (m_state.m_myPlanetsWithShips.Count > 0)
      {
        ...
        for (int i = 0; i < m_state.m_myPlanetsWithShips.Count; ++i)
        {
          ...
          if (targetPlanet.m_owner == 0 && targetPlanet.m_originalFuture.m_eventualOwner == 2)
          {
            if (distances[pid] < targetPlanet.m_originalFuture.m_future.Count)
            {
              if (targetPlanet.m_originalFuture.m_future[distances[pid]].m_owner == 0)
              {
                continue;
              }
            }
